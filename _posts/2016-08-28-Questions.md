...
### <span id="CSSAbout08"></span>8、块级元素的相关特点 ###

**包含块：**由最近的块级祖先框、表单元格或行内块祖先框的**内容边界**构成

**正常流：**指西方语言文本从左向右、从上向下显示，这也是传统HTML文档的文本布局。要让一个元素不在正常流中，唯一的办法就是使之成为浮动或定位元素

**非替换元素：**如果元素的内容包含在文档中，则称之为非替换元素

**替换元素：**指用作为其他内容占位符的一个元素（如img元素和大多数表单元素`<input type="radio">`）

**根元素：**位于文档树顶端的元素。在HTML中，是元素html。在MXL中，则可以是该语言允许的任何元素

**> 块级元素：**指段落、标题或div之类的元素。在正常流中，会在其框之前和之后生成**换行**，所以处于正常流中的块级元素会垂直摆放

包括：外边界、外边距、边框、内边距、内边界、内容区

- 内容区的背景会应用到内边距和边框（如果边框样式是有缝隙的，则可以看到元素的背景）
- 外边距通常是透明的，从中可以看到父元素的背景（边框的宽度不能是百分数只能是长度）
- 内边距不能是负值，但外边距可以（做外边距为负不仅会超出父元素边框，还会超出浏览器窗口本身的边界）
- 边框如果没有设置颜色，将取元素的前景色color（和文本一个颜色）

**width：**定义为从左内边界到右内边界的距离

- width影响的是**内容区**的宽度，而不是整个可见的元素框
- 当指定了内边距、边框或外边距这些会增加到可见框上，内容区并没有变
- 可以通过`box-sizing`来改变width是指内容宽度（`content-box`）还是可见框宽度（`border-box`）
- 正常流中，块级元素框的水平部分总和就等于父元素的width
...
这七个属性加在一起必须是包含块的宽度，即**包含块的width**（块级元素的父元素几乎都是块级元素）

其中能设置为**auto**的：width（auto或非负值）、margin-left、margin-right

正常流中**非替换块级元素**的水平格式化：

1. 如果width、margin-left或margin-right中的某个值为auto，而余下两个属性指定为特定值，那么设置为auto的属性会确定所需的长度，从而使元素框的宽度等于父元素的width
2. 如果所有这三个属性都设置为非auto的特定值（这些格式化属性过分受限），那么此时总会把margin-right强制为auto（对于从左向右读的语言是margin-right，对于从右向左读的语言则相反为margin-left）
3. 如果两个外边距都显式设置为某个值，width为auto（或不设置），则width将设置为所需的某个值，从而达到父元素的width
4. 如果两个外边距都设置为auto，而width为某值，则会使元素在其父元素中居中（老式浏览器会将外边距重置为0）
5. 某个外边距和width为auto，另一个外边距为具体值，则设置为auto的外边距会减为0，width会增加以达到父元素的width
6. 如果三个属性都设为auto，则两个外边距会置为0，width会尽可能宽达到父元素width（与默认情况相同：没有为外边距和width申明任何值，则外边距默认为0，width默认为auto）

**替换块级元素**的水平格式化和非替换元素基本一样，除了：如果width为auto，元素的宽度则是内容的固有宽度

**height：**定义为从上内边界到下内边界的距离

- 定义的是内容区的高度，不是可见框的高度
- 一个元素的默认高度由其内容决定
- 可以显示设置高度，如果指定高度大于内容所需高度，则多余的高度就好像是额外的内边距一样；如果指定高度小于内容所需高度，则向元素增加一个滚动条（取决于overflow属性的值）

**垂直属性：**垂直格式的七大属性：

    `margin-top、boder-top、padding-top、height、padding-bottom、border-bottom、margin-bottom`

这七个属性值加起来等于元素包含块的height

其中能设置为**auto**的是：height（auto或是非负值）、margin-top、margin-bottom

- 正常流中上下外边距设置为auto，会自动计算为0。这就不能使元素垂直居中
- height设置为百分数，这个值则是包含块height的一个百分数
- height设置为auto，显示时其高度将恰好足以包含其内联内容（包括文本）的行盒
- height设置为auto，且只有块级子元素，则其高度是从最高块级子元素的**外边框**边界到最低子元素外边框边界之间的距离。因此子元素的外边距会超出这些包含这些子元素的元素
- 如果块级元素有上内边距或下内边距，或上边框或下边框，则其高度是从最高子元素的上**外边距**边界到其最低子元素的下外边距边界之间的距离
- **合并垂直外边距：**两个外边距中较小的一个会被较大的一个合并
	- 如果垂直外边距都设为负值，则取两个外边距绝对值最大的
	- 如果一个正外边距和一个负外边距，则会从正外边距减去负外边距的绝对值
	- 如果由于负外边距导致元素重叠，则按先后顺序，后出现的元素覆盖较早出现的元素

**> 行内元素：**指em或span之类的元素（a是非替换行内元素，img是替换行内元素）。不会再之前或之后生成**行分隔符**，他们是块级元素的后代

**匿名文本：**指所有未包含在行内元素中的字符串，空格也是匿名文本

**em框：**em框在字体中定义，也称为字符框，实际的字形可能比其em框更高或更矮

**内容区：**在非替换元素中，内容区有两种：1、元素中各字符的em框串在一起构成的框  2、元素中字符字形描述的框

**行间距：**font-size值和line-height值之差，这个差分为两半，分别应用到内容区的顶部和底部

**行内框：**这个框通过向内容区增加行间距来描述。非替换元素：元素行内框的高度刚好等于line-height的值。替换元素：元素行内框的高度恰好等于内容区的高度，因为行间距不应用到替换元素

**行框：**包含该行中出现的行内框的最高点和最低点的最小框。行框的上边界要位于最高行内框的上边界，行框的底边要放在最低行内框的下边界

- 非替换元素的内边距、边框和外边距对行内元素或其生成的框没有垂直效果（不影响行内框的高度，也不影响包含该元素的行框的高度）
- 替换元素的外边距和边框确实会影响该元素行内框的高度，也可能影响包含该元素行框的高度

**确定行中个元素行内框的高度的步骤：**

- 得到个行内非替换元素及不属于后代行内元素的所有文本的font-size值和line-height值，再将line-height减去font-size，得到框的行间距。这个行间距除以2，将其一半分别应用到em框的顶部和底部
- 得到各替换元素的margin-top、boder-top、padding-top、height、padding-bottom、border-bottom、margin-bottom值，把它们加在一起
...
#### <span id="JSAbout06_01"></span>> Obeject类型 ####
...
#### <span id="JSAbout06_02"></span>> Array类型 ####
...
#### <span id="JSAbout06_03"></span>> Date类型 ####
